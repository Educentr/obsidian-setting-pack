...
task
FROM "MeetingNotes/121"
WHERE !completed and contains(join(people), regexreplace(this.file.name, "[- ]\d+", "")) and due < this.date
GROUP BY date
```
```dataview
task
...
```dataview
task
WHERE !completed and due <= striptime(this.date) and (contains(text, this.people.file.name) or contains(meta(section).subpath, this.people.file.name) )
